(function () {
  ...
  const PHOTOS = window.TML_PHOTOS || [];
  ...
  function buildMobileScroll() {
    ...
    const SCENES = ["Selected Work", "Johannesburg", "Cape Town", "India", "Nature", "Street"];
    ...
    PHOTOS.forEach((p, i) => {
      ...
      card.href = "Scene.html?cat=" + encodeURIComponent(p.h);
    ...
    /* fable-style bulge on the 2D list: photos swell inside their frames
       with scroll velocity via a CSS var, easing back when the scroll
       settles. */
    let lastY = window.scrollY || 0, velY = 0, stretching = false;
    function stretchLoop() {
      velY *= 0.88;
      const s = 1 + Math.min(0.14, Math.abs(velY) * 0.0016);
      if (s > 1.004) {
        track.style.setProperty("--vbulge", s.toFixed(3));
        requestAnimationFrame(stretchLoop);
      } else {
        track.style.setProperty("--vbulge", "1");
        stretching = false;
      }
    }
    window.addEventListener("scroll", () => {
      const yNow = window.scrollY || 0;
      velY = velY * 0.7 + (yNow - lastY) * 0.3;
      lastY = yNow;
      if (!stretching) { stretching = true; requestAnimationFrame(stretchLoop); }
    }, { passive: true });
  }
  ...
  /* organic bulge — spring-driven swell factor fed by scroll speed */
  let sm = 0, smV = 0;
  ...
  function render(now) {
    ...
    /* fable-style organic bulge: an underdamped spring chases scroll
       velocity, so frames swell as motion picks up and settle back with a
       soft, natural overshoot — never a hard linear stretch. */
    const sTgt = Math.min(0.22, Math.abs(vel) * 1.1);
    smV += (sTgt - sm) * 0.16;
    smV *= 0.8;
    sm += smV;
    if (sTgt === 0 && Math.abs(sm) < 0.0008 && Math.abs(smV) < 0.0008) { sm = 0; smV = 0; }
    ...
    const moving = (f !== target) || (pmx !== mx) || (pmy !== my) || popActive || sm !== 0 || smV !== 0;
    ...
    for (let i = 0; i < N; i++) {
      ...
      /* bulge is strongest at the screen-centre frame and melts away with
         distance — a radial swell moving through the coil, not a stretch */
      const bell = Math.exp(-(d * d) * 0.35);
      const bulge = Math.max(-0.06, sm) * bell;
      const ps = (1 + pop[i] * 0.12) * mScale * (1 + bulge * 0.3);
      ...
      /* the photo itself swells inside its frame — the lens-bulge read */
      const iz = "scale(" + (1 + bulge).toFixed(3) + ")";
      if (iz !== cI[i]) { imgs[i].style.transform = iz; cI[i] = iz; }
